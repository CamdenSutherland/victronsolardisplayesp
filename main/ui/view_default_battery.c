...
ui_device_view_t *ui_default_battery_view_create(ui_state_t *ui, lv_obj_t *parent)
{
    ...
    lv_obj_set_style_pad_all(view->left_column, 5, 0);
    ...
    lv_obj_set_style_pad_all(view->center_column, 5, 0);
    ...
    lv_obj_set_style_pad_all(view->right_column, 5, 0);
    ...
    lv_obj_set_style_pad_bottom(view->dcdc_input_voltage_label, 5, 0);
    ...
    lv_obj_set_style_pad_bottom(view->dcdc_output_voltage_label, 5, 0);
    ...
    lv_obj_set_size(view->arc_container, 160, 160);
    ...
    lv_obj_align(view->arc_container, LV_ALIGN_CENTER, -18, 0);

    // Create SOC arc (smaller size)
    view->soc_arc = lv_arc_create(view->arc_container);
    lv_obj_set_size(view->soc_arc, 160, 160);
    ...
    lv_obj_set_style_arc_width(view->soc_arc, 15, LV_PART_MAIN);
    lv_obj_set_style_arc_color(view->soc_arc, lv_color_hex(0x333333), LV_PART_MAIN);
    
    // Style the arc indicator (progress)
    lv_obj_set_style_arc_width(view->soc_arc, 15, LV_PART_INDICATOR);
    ...
    lv_obj_align(view->soc_label, LV_ALIGN_CENTER, 0, -15);  // Move up more to make room for voltage
    ...
    lv_obj_align(view->battery_voltage_label, LV_ALIGN_CENTER, 0, 15);  // Position below SOC
    ...
    lv_obj_set_style_pad_top(view->ttg_label, 2, 0);  // Minimal padding
    ...
    lv_obj_set_style_pad_top(view->power_consumption_label, 2, 0);  // Minimal padding
    ...
    if (battery_view->battery_state.has_data && 
        ...
        if (battery_view->battery_state.battery_voltage_cv > 0 && battery_view->battery_state.battery_current_milli != 0) {
            ...
            if (power_watts >= 0) {
                ...
                lv_label_set_text_fmt(battery_view->power_consumption_label, "Power: %ldW", (long)power_watts); // Negative indicates charging
            }
        } else {
